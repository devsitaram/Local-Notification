...
fun NotificationViewScreen() {
    ...
    Column(
        ...
        // timer to set
        var remainingTime by remember { mutableStateOf(120) } // 120 seconds = 2 minutes
        ...
        if (remainingTime == 0) {
            showNotification(
                context = context,
                description = notificationMessage.ifEmpty { "This is a notification" })
        } else {
            Text(text = "Remaining Time: ${formatTime(remainingTime)}", fontSize = 24.sp)
        }
    }
}
...
//@Composable
//fun TimerApp() {
//    // Create a mutable state for displaying the remaining time in seconds
//    var remainingTime by remember { mutableStateOf(120) } // 120 seconds = 2 minutes
//
//    val countDownTimer = rememberUpdatedState(
//        object : CountDownTimer((remainingTime * 1000).toLong(), 1000) {
